...
function getLocationCode() {
    ...
    getDataPayObligation("tableInterest", '"' + arrayJson[0] + '":"' + arrayJson[1] + '"', 0);
}
//**Function get Obligation **/
//Author: DIEGO CASALLAS
//Date: 21/11/2020
...
function getDataPayObligation(table, dataSetObligation, typeSend) {
    try {
        loadPageView();
        var xhttp = new XMLHttpRequest();
        var arrayInterest = new Array("DTF","FECHA","IBR");
        var JsonData;
            ...
            if (this.readyState === 4 && this.status === 200) {
                ...
                if (xhttp.responseText != "") {
                    ...
                    if (jsonObj.length != 0) {
                        ...
                        if (typeSend == 0 ) {
                            //console.log(jsonObj);
                            tableInterest = new Table(table, arrayInterest, jsonObj);
                            tableInterest.createTableInterest();
                        } 
...
function sendData(idForm, e) {
    ...
    if (validatorForm(idForm)) {
        ...
        //console.log(jSon);
        //getDataPayObligation("tableInterest",jSon,4);
    } else {
...
function searchObligation(e) {
    try {
        ...
        getDataPayObligation("tableInterest", jsonData, 1);
    } catch (error) {
        console.error(error);
    }
    e.preventDefault();
...
function convertToExcel(){
    ...
    window.open('https://docs.google.com/spreadsheets/d/1DsK9WxzRZ7kNj6DIsaZpEGnoFuM7QwjGuaCAQ48sMq4/edit#gid=0');
}